...
(function () {
  ...
  /** Legacy 10 AM slot labels still saved on some volunteer rows. */
  var LEGACY_TIME_ALIASES = {
  ...
  var DAY_TITLE = {
    ...
    'June 26, 2026': 'June 26, 2026 (Friday) · 6–10 PM',
  ...
  function findSlotKeyForLine(date, time) {
    var normalized = LEGACY_TIME_ALIASES[time] || time;
    var k;
    for (k in SLOT_META) {
      if (SLOT_META[k].date === date && SLOT_META[k].time === normalized) {
        return k;
      }
    }
    return null;
